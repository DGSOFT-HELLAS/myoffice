...
const MessageRequest = ({data, setRefresh}) => {
  ...
  const handleFetch = async () => {
    const response = await fetchAPI(
      ...
      {query: 'getRequest', soaction: data.callID},
    );
      ...
      if (response[0].remarks) {
        setState(prev => {
          return {
            ...prev,
            data: response[0],
            text: response[0].remarks ? response[0].remarks : '',
          };
        });
      }
    } catch (error) {
  ...
  const onPressSaveMessage = async () => {
    ...
    const response = await fetchAPI(
      'https://portal.myoffice.com.gr/mobApi/queryIncoming.php',
      {query: query, callID: data.callID, remarks: state.text},
    );
    ...
    <View style={styles.iconView}>
      {data.status == 0 && (
        <TouchableOpacity
          ...
          style={[styles.icon, styles.fileIcon]}>
