  ...
  return (
    <DrawerContentScrollView {...props} contentContainerStyle={{flex: 1}}>
      <View style={styles.headerView}>
        <BoldText style={{fontSize: 28, color: COLORS.contrastText}}>
          MyOffice Services
        </BoldText>
        <Text style={{fontSize: 17, color: COLORS.secondaryColor}}>
          WE SIMPLIFY YOUR WORK LIFE
        </Text>
      </View>
        ...
        <View style={styles.tabContainer}>
          ...
          {id == 2 && (
            <DrawerSubItemView>
              <DrawerSubItem
                title="Κλήσεις"
                screen="IncomingCallsForm"
                setId={setId}
              />
              <DrawerSubItem
                title="Email"
                screen="IncomingEmailForm"
                setId={setId}
              />
              <DrawerSubItem
                title="Εlta"
                screen="IncomingEltaForm"
                setId={setId}
              />
              <DrawerSubItem
                title="Αναφορά Εργασίας"
                screen="IncomingTaskForm"
                setId={setId}
              />
            </DrawerSubItemView>
          )}
        ...
        <View style={styles.tabContainer}>
          ...
          {id == 3 && (
            <DrawerSubItemView>
              <DrawerSubItem
                title="Πελάτες"
                screen="CustomerSearchForm"
                setId={setId}
              />
              <DrawerSubItem
                title="Προσθήκη Πελάτη"
                screen="Προσθήκη πελάτη"
                setId={setId}
              />
            </DrawerSubItemView>
          )}
...
const DrawerItem = ({title, Icon, iconName, index, setId, id, homePage}) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={homePage ? goToHome : handleCollapse}>
...
const DrawerSubItemView = ({children}) => {
  ...
  return (
    <Animated.View style={[{opacity: fadeAnim}, styles.subItemView]}>
      {children}
    </Animated.View>
  );
};
